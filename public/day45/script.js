...
const predictions = [
    ...
    const sec_msg = `Your zodiac sign is ${zodiacSigns[month-1]}`;
    const third_msg = `Compliment: ${compliments[day-1]}`;

    const ind = Math.floor(Math.random() * 20);
    const forth_msg = `Victim play: ${victimCardCompliments[ind]}`;

    const index = (name.length * surname.length * year) % 30;
    const fifth_message = `Our Recommendation: ${recommendations[index]}`;

    const ind2 = (day * month * year) % 20;
    const sixth_message = `Your Future Prediction: ${predictions[ind2]}`;

    const result = document.getElementById('result');
    result.innerHTML = `${first_msg} <br> ${sec_msg} <br> ${third_msg} <br> ${forth_msg} <br> ${fifth_message} <br> ${sixth_message}`;
});